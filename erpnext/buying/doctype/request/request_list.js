...
function open_bulk_upload_dialog(listview) {
    ...
    function parse_csv_preview(content) {
        ...
        frappe.call({
            method: "erpnext.buying.doctype.request.request.parse_forecast_upload",
            args: {
                csv_content: content
            },
            callback: function(r) {
    ...
    function render_preview(data) {
        ...
        data.groups.forEach(function(group, group_idx) {
            ...
            html += '<th style="padding: 6px 8px; text-align: left; border-bottom: 1px solid #dee2e6;">' + __('UOM') + '</th>';
            ...
            group.items.forEach(function(item, item_idx) {
                ...
                html += '</td>';
                html += '<td style="padding: 6px 8px; border-bottom: 1px solid #eee;">' + (item.uom || '-') + '</td>';
        ...
        $preview.find('.bulk-qty-input').on('change', function() {
            ...
            const rate = parsed_data.groups[group_idx].items[item_idx].rate || 0;
            const amount = new_qty * rate;
            $preview.find('.bulk-amount[data-group="' + group_idx + '"][data-item="' + item_idx + '"]').text(amount.toFixed(2));

            // Update data model
            parsed_data.groups[group_idx].items[item_idx].qty = new_qty;
