...
def get_sales_invoice_rows():
    rows = frappe.db.sql(
        """
        SELECT si.name, si.posting_date, sii.warehouse, sii.item_code,
               sii.item_name, sii.qty, sii.rate, sii.amount
        ...
        ORDER BY si.posting_date, si.name, sii.warehouse, sii.item_code
        """,
    ...
    for row in rows:
        ...
        result.append([
            len(result) + 1, row.name, "Return" if qty < 0 else "Invoice",
            str(row.posting_date), row.item_code, row.item_name, abs(qty),
            flt(row.rate), flt(row.amount), rate, flt(abs(qty) * rate),
        ])
    return result
