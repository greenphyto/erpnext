...
frappe.ui.form.on("Delivery Note", {
	...
	refresh: function(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.is_return === 1 && frm.doc.per_billed == 100 && frm.doc.doctype == "Delivery Note") {
...
erpnext.stock.DeliveryNoteController = class DeliveryNoteController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		if (!doc.is_return && doc.status!="Closed" && doc.doctype == "Delivery Note") {
			if(doc.docstatus == 1) {
		...
		if (doc.doctype == "Delivery Note") {
			...
			if(doc.docstatus==1 && !doc.is_return && !doc.auto_repeat && doc.status!="Closed" && doc.doctype == "Delivery Note") {
			...
			me.frm.set_query("item_code", "items", function(doc, cdt, cdn) {
				...
				if (!me.frm.doc.non_package_item){
