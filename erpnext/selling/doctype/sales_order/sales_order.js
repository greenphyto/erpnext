...
erpnext.selling.SalesOrderController = class SalesOrderController extends erpnext.selling.SellingController {
	...
	refresh(doc, dt, dn) {
		...
		this.order_type(doc);
		this.pending_po();
		erpnext.selling.render_delivery_progress(this.frm);
	}

	pending_po(refresh=false){
		if (this.frm.doc.is_pledge) return;
		if (this.frm.doc.pending_po){
			...
			this.frm.set_df_property("po_no", "hidden", 1);
...
erpnext.selling.render_delivery_progress = function(frm) {
    // 1. Calculate total Qty and Delivered Qty from child table 'items'
    let total_qty = 0;
    ...
    // 5. Insert into field donor_info
