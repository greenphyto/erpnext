...
def update_voucher_outstanding(voucher_type, voucher_no, account, party_type, party):
	...
	if (
		...
		ref_doc.outstanding_amount = flt(outstanding.get("outstanding_in_account_currency"))
		frappe.db.set_value(
			voucher_type, voucher_no, "outstanding_amount", flt(outstanding.get("outstanding_in_account_currency"))
		)
...
def get_cost_center_from_account(account, company=""):
	...
	cost_center = erpnext.get_default_cost_center(company=company, account=account)
	if cost_center:
		return {"value":cost_center, "lock":1}

	# based on Cost Center Mapping Flow 6/1/25
